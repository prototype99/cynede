...
inherit bash-completion-r1 gnome2-utils rpm versionator unpacker user

DOWNLOAD_PAGE="http://www.bitdefender.com/site/Downloads"
SRC_NAME_BASE="BitDefender-Antivirus-Scanner-$(replace_version_separator 2 '-')-linux-ARCH.rpm.run"
...
SRC_URI="amd64? ( ${SRC_NAME_BASE/ARCH/amd64} )"
...
DONE
	...
	cd "${D}"/$DIR/bin && for bin in *; do
		dosym /$DIR/bin/${bin} /opt/bin/${bin}
	done

	dodir /etc/$BASE
	dosym /$DIR/etc/certs /etc/$BASE/certs
